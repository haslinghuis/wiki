...
The gyro loop rate is now locked at the 'native' gyro sampling rate.  This rate cannot be adjusted by the user.  The PID loop rate can, and should, be adjusted to suit the capabilities of the processor.  F405 and F7xx boards should be fine with 8k PID loops, but F411's should be set to 4k.  Custom F3 builds can run 4k but are happier at 2k.

When running 4k or 2k PID loops, aliasing artefacts from down-sampling the 8k gyro data to the PID loop frequency can occur.  This is why the gyro lowpass 2 filter runs at gyro sampling rate, and is enabled by default.  It provides both noise reduction and antialiasing benefits.  If it is disabled, a simple 2-point averaging filter is automatically enabled in its place.  This reduces aliasing artefacts in 4k PID loops with a little less delay, but does not prevent aliasing at 2k.  Hence lowpass 2 should not be disabled with 2k PID loops. 

With an 8k PID loop, set gyro lowpass 2 to whatever works best for noise reduction; 8k PID loops do not have aliasing concerns.
...
- dshot600 -> 8k max pidloop (at 8k, Dshot 300 data only gets sent every second PID loop)

Without bidirectional enabled:  
- dshot150: 4k max pidloop  
- dshot300: 8k max pidlopp  
- dshot600: 16k max pidloop (16k not possible in 4.2)
