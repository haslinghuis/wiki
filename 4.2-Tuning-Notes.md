...
Betaflight 4.2 brings solid improvements in flight performance.  Most users will find their quad flies smoother due to [feed-forward improvements](4.2-Tuning-Notes#Improved-Feed-Forward).  Feed forward settings such as averaging and smoothing can be optimised for race to cinematic purposes.  More [accurate loop times](4.2-Tuning-Notes#More-accurate-loop-times) have greatly improved RPM filter accuracy.  A [dynamic D expo curve](4.2-Tuning-Notes#Dynamic-D-filtering-expo) improves propwash by raising D filter cutoffs faster.  Dynamic [battery sag compensation](4.2-Tuning-Notes#Dynamic-nattery-sag-compensation), when enabled, provides consistent throttle and PID 'feel' throughout the flight.  New rates methods, when used with the latest Configurator, provide an easy way to adjust centre stick feel independently of expo and maximum roll rate.  Yaw spins after striking objects [should stop more quickly](4.2-Tuning-Notes#Automatic-anti-yaw-spin-configuration). [RC smoothing defaults]](4.2-Tuning-Notes#RC-smoothing-improvements) are much improved and shouldn't need to be manually tweaked anymore.  
...
**The [latest 10.7](https://github.com/betaflight/betaflight-configurator/releases) [Betaflight Configurator](https://github.com/betaflight/betaflight-configurator) is mandatory.  Earlier versions are not going to work properly.**
...
- filtering D quite a bit harder than usual (2 clicks left on the D filter slider, at least)
- avoiding high P (adjust the P and D gain slider to whatever minimises the wobble, trial and error)
- about half the usual default I on pitch and roll
