...
An accurately clocked PID loop significantly improves filter accuracy, especially with RPM and dynamic notch filtering.  This should lead to cooler motors and smoother flight.

The gyro loop rate is now always the 'native' gyro sampling rate.  The only filter that runs at gyro loop rate is gyro lowpass 2; everything else at PID loop rate.  

The PID loop rate can still be set to suit the capabilities of the processor.  F405 and F7xx boards should be fine with 8k PID loops, but F411's should be set to 4k.  The gyro loop rate cannot be changed.
