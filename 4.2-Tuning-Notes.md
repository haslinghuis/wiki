...
The result was that, bounce back would not have been effectively controlled by lowering `iterm_relax_cutoff` in setpoint mode, but not gyro mode.  This is perhaps why, since version 3.5, many freestyle pilots changed to gyro mode.

Now, setpoint mode works correctly when the user customises the `iterm_relax_cutoff` value. 
