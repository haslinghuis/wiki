# [Betaflight 4.2](https://github.com/betaflight/betaflight/releases) brings solid improvements in flight performance:

- [More accurate loop times](4.2-Tuning-Notes#More-accurate-loop-times) - improves RPM filter performance
- [Improved feed forward](4.2-Tuning-Notes#Improved-Feed-Forward) - helps your quad fly smoother, with snippets to [optimise averaging and smoothing settings for[race to cinematic purposes](4.2-Tuning-Notes#Quick-settings)
- [Dynamic D filter expo curve](4.2-Tuning-Notes#Dynamic-D-filtering-expo) - improves propwash by raising D filter cutoffs faster as you throttle up 
...
**4.2 should fly really well on defaults, with no modifications, with most quads.  If you know your quad requires special PIDs, please re-tune, but start from PIDs you know well.  With all 4.2 features active a minor re-tune from earlier PIDs may get you an even better result.  For instance, if you wanted to be able to add more D but were limited by noise, it may be possible now.  Some low authority quads (under-powered whoops, ducted HD whoops, 7" and larger quads, endurance quads, etc) may require [specific settings to minimise bounce-back](4.2-Tuning-Notes#Settings-to-minimise-bounceback).**
...
- About 20% less P than your normal freestyle tune, just barely enough to provide basic P stability.
- about half the default I on pitch and roll
- high FF transition
- use Actual Rates with centre set to 10, expo to 0, and your usual max rate.  Actual with zero expo gives a soft centre with quicker return to normal responsiveness out of centre.  Transition and/or deadband may not be required when the centre sensitivity is very low.
...
## Settings to minimise bounce-back
...
The PID `I` term, in Betaflight, is what provides precision in tight turns, pitch accuracy in fast forward flight, stability on windy days, stability in drops and mid-flip, asymmetric aero drag effects, and generally cleans up the myriad small persistent residual errors that P can't entirely deal with.  `I` accumulates a correction for small residual errors over time, and that correction keeps the quad exactly on track.  Higher `I` values will do this faster and more completely, especially in tight turns and on windy days, where there are small persistent residual errors that P, D or FF are unable to control.  One of the reasons that Betaflight 4.x tracks so accurately in tight turns and on long straights is the relatively high I value.

At some point, however, the pilot will request a change in turn rate that is too quick for the quad to deliver.  During this period the gyro signal will lag setpoint, there will be a large error signal, and `I` will start to accumulate in order to try to help correct this error, unless this is controlled by the software.  If the software control is inadequate for the amount of error, a large amount of I may accumulate, and when the pilot stops the flip, all that accumulated I now drives an error in the reverse direction, which slowly fades away, until it gets back to zero.  

The pilot sees a single, fairly slow, 'bounce-back' event when they terminate the flip.

This phenomenon is called 'iterm windup'.

To prevent bounce-back from iterm windup, we use the `iterm_relax` and `iterm_windup` settings.  The defaults for these parameters work very well in 4.2 for 'normal authority' builds.  `iterm_relax`, at default of 15, strongly suppresses I accumulation during normal flip and roll inputs, and `iterm_windup` isn't needed, usually, so is off by default.

'Low-authority' quads will tend to get bounce-back after quick flips or rolls, and after quick yaw inputs, because they have significant delay between setpoint requests and when the quad actually responds.  The Betaflight default values for `iterm_relax` and `iterm_windup` will not be optimal for these machines.  
...
1.  The best way is to change the tune or modify the hardware of the quad to make it more responsive.  This will improve the overall handling of the machine and reduce the bounce-back.  

Low-authority machines typically require higher P and FF.  Maybe up to double the default P and triple the default FF.  There is a limit to how high you can go, but you need to go high.  Most likely D needs to go up as well, and D noise may be the limiting factor.  You should push FF hard, and move the 'P and D gain' slider progressively to the right.  This should reduce the wobble and the bounce back because it will get the machine turning more quickly and reduce error magnitude and duration.  Note that very high D will delay responsiveness and make bounce-back worse.  You should find that wobble will be better once P and D are at the right values.

In some cases, the limit on how high you can go with P and D is reached and still you have wobble and bounce-back.  You should then reduce I, trying half normal or a third of normal. Very low I will make the quad less stable overall, so don't go too low.

Over-propped quads will have difficulty changing thrust quickly.  Always try smaller or lighter props that the motors can spin up more readily.  Optimising the prop to the motors can make a huge difference.  

On very large quads, beast or X-class, rotating the motors inwards will improve yaw authority.

Any change that improves bounce-back means that you've got the quad responding more quickly and precisely. 

2.  The next best way is to reduce your maximum pitch, roll or yaw rate.  Sometimes lowering the maximum requested rate, or just flying more smoothly, entering or leaving the flips or yaws a little more slowly, is all that's needed.  This way you can keep the stability that I provides.

3.  The 'software' way: tuning the provided `iterm_relax` and `iterm_windup` functions.

`iterm_relax` is what prevents I accumulation when the sticks move quickly.  By default it applies only to pitch and roll.  

The associated `iterm_relax_cutoff_hz` value determines the strength of the suppression strong the suppression effect, and how long it lasts for.  Lower values suppress `I` accumulation more strongly and for longer. 

To control bounce-back on pitch and roll, reduce `iterm_relax_cutoff_hz` from the default of 15 to 10, then 7, then 5, noting the improvement in bounce-back each time.  Settle on the highest value that 'works'.  The lower you go, the less accurate your tighter turns will become.  So there is a bit of a trade-off to consider.  

We strongly recommend keeping iterm relax in its default setpoint mode.  There is no need to change to gyro mode in 4.2.  Iterm relax will likely perform better in setpoint mode than gyro mode in 4.2. 

`iterm_windup` in 4.2 now applies only to yaw, and is what should be used to control yaw bounce-back. It suppresses `I` accumulation whenever the motor differential exceeds the set percentage.  By default it is 100, meaning 'off'.  

When we `set term_windup = 70`, iTerm accumulation will cease whenever the motor differential required to drive the yaw exceeds 70%.  This is a really effective way to control yaw bounce-back and yaw jump on larger quads and those ducted quads that lack yaw authority.  It will reduce high rate yaw responsiveness, but allow quick yaw reactions to smaller inputs - they will be completely unaffected.  
...
CRSF link quality now displayed as A:BB where A is the RF mode (2=150hz, 1 = 50hz), and NN is link quality.
...
- Configurator - mgvivergim, IvoFPV, fgiudice98, Asizon, IllusionFpv
