...
The basic gyro and PID loop scheduling has been completely rewritten, resulting in much less PID loop jitter and reduced CPU utilisation.  No special configuration is required; it just works.

An accurately clocked PID loop significantly improves filter accuracy, especially RPM and dynamic notch filtering accuracy.  This should lead to cooler motors and smoother flight, or alternatively let you set the RPM filter Q factor to a higher value (reducing delay) while maintaining good motor noise rejection.

The gyro loop rate is now locked at the 'native' gyro sampling rate, and this cannot be adjusted by the user.  The only filter that now runs at gyro loop rate is gyro lowpass 2; everything else at PID loop rate.  
...
When running 4k or 2k PID loops, we need to prevent aliasing artefacts.  These arise at the point where we down-sample the 8k gyro data to the PID loop frequency.  The gyro lowpass 2 filter, if active, acts as a very effective anti-aliasing filter.  If gyro lowpass 2 is disabled, a simple 2-point averaging filter is automatically activated in its place, and also runs at gyro loop frequency.  The 2-point averaging filter will reduce, but not totally eliminate, aliasing artefacts in 4k PID loops.  It does not prevent aliasing artefacts in 2k PID loops.  

If you run 4k or lower PID loops we recommend retaining the gyro lowpass2 filter.  If you don't need gyro lowpass 2 for noise reduction purposes, set it as high as it can go (currently to 1000Hz) to provide the anti-aliasing functionality.
...
With rpm filtering, the motor noise lines will usually be removed completely, and often the dynamic notch has little to do.  In many cases it can be turned off.  However, it can be useful for attenuating residual noise or frame resonance.  If there is no particular frame resonance, it will sort of hover around the middle of its range.  With the defaults, this keeps it out of the way, causing relatively little delay.  
...
`dyn_lpf_curve_expo` now makes the D lowpass rise more quickly with throttle increases than it used to.  Also, the rise in D lowpass now starts from the moment the throttle is increased.  This may improve propwash control.

A suitable value to start with is 5-7.

The end-points, `dyn_lpf_dterm_min_hz` and `dyn_lpf_dterm_max_hz`, remain the same, you just get closer to max more quickly.

Default is no change (0).  Higher values may improve propwash by reducing delay at mid-throttle.  This can be particularly useful when solid D filtering is needed at idle, but not so much is needed once the motors get going.  
