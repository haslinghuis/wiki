...
**NOTE 2**: If you previously had `dyn_notch_range` set to `LOW` or `HIGH`, you may want to change `dyn_notch_max_hz` to 350 or 700 - details [here](4.2-Tuning-Notes#Improved-dynamic-notch-filter).
...
Previously, the anti-gravity gain value acted as a multiplier of the PID `I` value.  This means that when PID `I` was increased, the strength of the anti-gravity effect also increased, and vice versa.  This could lead to I mediated wobbles on rapid changes of throttle when I was increased to high values.

In 4.2, the anti-gravity effect is independent of the user's PID `I` value, and can be varied by adjusting anti-gravity gain alone.  
