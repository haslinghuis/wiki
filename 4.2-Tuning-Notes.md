...
- [Dynamic battery sag compensation](4.2-Tuning-Notes#Dynamic-battery-sag-compensation)
- [New rates modes](4.2-Tuning-Notes#New-Rates-modes)
...
- [NFE Race Mode](4.2-Tuning-Notes#NFE-Race-Mode)
- [Configurator 10.7](4.2-Tuning-Notes#Configurator-improvements)
- [OSD Improvements](4.2-Tuning-Notes#OSD-improvements)

[Betaflight 4.2](https://github.com/betaflight/betaflight/releases) brings solid improvements in flight performance.  

Most users will find their quad flies smoother due to [feed-forward improvements](4.2-Tuning-Notes#Improved-Feed-Forward).  Feed forward settings such as averaging and smoothing can be [optimised for race to cinematic purposes](4.2-Tuning-Notes#Quick-settings).  

More [accurate loop times](4.2-Tuning-Notes#More-accurate-loop-times) have greatly improved RPM filter accuracy.

A [dynamic D expo curve](4.2-Tuning-Notes#Dynamic-D-filtering-expo) improves propwash by raising D filter cutoffs faster.  

Dynamic [battery sag compensation](4.2-Tuning-Notes#Dynamic-nattery-sag-compensation), when enabled, provides consistent throttle and PID 'feel' throughout the flight.  

New rates methods, when used with the latest Configurator, provide an easy way to adjust centre stick feel independently of expo and maximum roll rate.  

Yaw spins after striking objects [should stop more quickly](4.2-Tuning-Notes#Automatic-anti-yaw-spin-configuration). [RC smoothing defaults]](4.2-Tuning-Notes#RC-smoothing-improvements) are much improved and shouldn't need to be manually tweaked anymore.
...
**NOTE 5**: If using the Accelerometer, arming will fail unless it has been calibrated.
...
- disabling D_min,
- setting D about 20% above P, using the P:D ratio slider
- setting TPA to D only, to start at your cruising throttle value, and increase the cut percentage a little
- setting the D lowpass expo value as high as possible (you'll be limited by mid-throttle D noise)
- likely with the extra D, you'll need to filter D a bit more than usual (eg 2 clicks left on the D filter slider)
- avoiding high P (use about 20% less than a normal freestyle tune, a value typically between 30-40, just barely enough to provide basic P stability).
- about half the default I on pitch and roll
- iterm_relax_cutoff_hz can go as low as 1
- high FF transition
- use Actual Rates with centre set to 120, expo to 0, and your usual max rate.  Adjust the centre rate to whatever softness you want around centre position.  Actual with zero expo gives a flatter centre with quicker return to normal responsiveness out of centre.  It does this better than any other rates mode for HD flying.
...
Gyro and PID loop scheduling has been completely rewritten, resulting in less PID loop jitter and reduced CPU utilisation.  No special configuration is required; it just works.
...
The gyro loop rate is now locked at the 'native' gyro sampling rate; it cannot be adjusted by the user.  Usually this is 8k.  The PID loop rate can, and should, be adjusted to suit the capabilities of the processor.  The PID loop time is displayed at the bottom of the Configurator.

F405 and F7xx boards may run well with 8k PID loops, but are most precise at 4k.  Check how accurately the loop time is in Configurator at 4k, then try 8k; only stay with 8k if it seems at least as stable.  

F411's should be set to 4k but typically will require overclocking to get optimal results.  

To keep jitter at the lowest possible value, don't log, and disable debugging, unless you need them.

When running 4k or 2k PID loops, aliasing artefacts from down-sampling the 8k gyro data to the PID loop frequency does occur.  The default-on gyro lowpass 2 filter runs in the gyro loop to provide antialiasing benefits, independent of its filtering function.  It is the only lowpass running at gyro loop speed.  If it is disabled, a simple 2-point averaging filter is automatically enabled in its place.  This reduces aliasing artefacts adequately in 4k PID loops, and has a tiny less delay than gyro lowpass 2 at 1000hz, but does not prevent aliasing at 2k.  That is why lowpass 2 should not be disabled with 2k PID loops. 

With an 8k PID loop, set gyro lowpass 2 to whatever works best for noise reduction; 8k PID loops do not have aliasing issues.
...
We've introduced two new Rates configuration modes, and have updated the Configurator to graphically display the results of all Rates configurations.  Using the latest Configurator is essential.
...
Both new methods let you directly set your intended maximum roll rate in degrees/second, and this won't change if you modify expo or the centre stick rate value.
...
`QUICK` Rates keeps the same rcRate value as used in previous Betaflight versions; you can keep the same value you've always used.  In `ACTUAL` Rates, the centre rate you want is set in degrees/second.  To convert your old rcRate value to degrees/second, multiply it by 200.  For example, an rcRate of 1.0 would be entered as a centre sensitivity of 200 in `ACTUAL` rates.  This feels like a straight line to max rate at 200 deg/s.  In `ACTUAL` rates, the centre and max numbers are directly comparable in terms of 'how will it feel' across the stick movement range.

Expo now only changes the point at which the sensitivity ramps up from centre to maximum stick position.  Higher expo will increase rates later in the stick travel.  Changing expo will not change centre responsiveness or max roll rate.  In `ACTUAL` rates, the amount of expo can be reduced below what was previously possible, which can be good for racers; at high expo values, the stick sensitivity can be constant for quite a wide range of stick input before rising only at the very end.  In `QUICK` rates, expo has a more traditional curve.
