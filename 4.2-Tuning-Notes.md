...
Please use the latest [Betaflight Configurator](https://github.com/betaflight/betaflight-configurator).  A [nightly build](https://github.com/betaflight/betaflight-configurator-nightlies/releases) is also available.

### NOTE 1: DO NOT paste a diff or dump from any build before 4.1 into the 4.2 CLI.  A 4.1 `diff all` should be OK.  If you had `dyn_notch_range` set to `LOW` or `HIGH`, you may want to change `dyn_notch_max_hz` - details [here](4.2-Tuning-Notes#Improved-dynamic-notch-filter). 

### NOTE 2: If you're new to 4.2, please start with stock filters and PIDs, unless you know for sure that your quad needs unusual settings. 
...
The main benefit is improved filter accuracy, especially for RPM and dynamic notch filtering.  This should lead to cooler motors and smoother flight.  Alternatively you may be able to set the RPM filter Q factor to a higher value, reducing delay, while maintaining good motor noise rejection.
