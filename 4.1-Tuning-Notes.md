...
Betaflight 4.1 significantly improves default filters, PIDs and FeedForward capabilities, all of which help make your quad fly better than ever, out of the box.  And we have a new configurator!
...
### NOTE 2: Start with stock filters and PIDs, unless you know for sure that your quad really needs something unusual.  The default PIDs are really, really good. 
...
In 4.0x, with rc_smoothing in auto mode (`set rc_interp = AUTO`), these filter values were set dynamically according to the step intervals between successive incoming RC data points.  
...
The default `ff_spike_limit` threshold of 40 works well in most cases.
