...
Betaflight 3.5 introduces the new Feed Forward PID system, makes Anti-Gravity work better, and allows customisation of the Dynamic Notch filter to better track motor noise and reduce delay in 'clean' quads, and many bug fixes.  

In practical terms these changes should:

- improve nose up/down wobble when changing throttle quickly, 
- improve yaw performance, and 
...
Feed Forward now takes over the role that D weight previously performed, and is now available on yaw.

The default values for FF are equal to the amount of D weight in the previous 3.4 defaults.  
