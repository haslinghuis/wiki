...
OpenTx has a specialised input filter that only transmits changes that are above a certain size, the ADC filter.  This results in sending no data for variable periods of time when the sticks move slowly.  This is intended to reduce jitter.  Unfortunately, anytime no data is sent, FF drops abruptly to zero.  The result is a very steppy FF trace and a reduction in fine control with small stick movements.  Disabling the ADC filter will cause the radio to transmit whatever data it has every packet.  This means transmitting some jitter but improves fine control.  The Hall effect gimbals have very little jitter and the ADC can be turned off without problems.

If you have a FrSky radio with Hall gimbals (not recommended if you have the stock potentiometer gimbals):
