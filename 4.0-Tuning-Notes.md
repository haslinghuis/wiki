...
- [Improved yaw PIDs](https://github.com/betaflight/betaflight/wiki/4.0-Tuning-Notes#Improved-yaw-PIDs), with a better understanding of how to tune yaw
- [Improved dynamic notch code](https://github.com/betaflight/betaflight/wiki/4.0-Tuning-Notes#Improved-dynamic-notch-code)has been improved to better track and suppress motor noise with less delay, using two closely spaced, narrower notches


Other flight-related changes include:

- Absolute control has been improved and is now enabled by default, at a value of 10, primarily for Freestyle/LOS rather than race purposes
...
Propwash happens when, after throttling down, the pilot then throttles up, and the quad is falling backwards/downwards into dirty air.  Any propeller in reversed airflow generates tip vortexes that greatly reduce thrust, and 'dirty' air itself may be very turbulent and buffet the quad as we fly out through it.  Whenever we cut throttle and deal with wobble, some motors will start at, or go to, lower rpm than others.  These motors will be slower to spin up than the others, leading to very unequal thrust development, causing crosstalk between axes, secondary PID reactions, and worsening the instability.  Thrust is not remotely linear to PIDs, and normal PID control is relatively ineffective.

The best solution for propwash is flying style.  A good pilot can fly a quad with terrible propwash behaviour so smoothly that it looks perfect.  The key piloting factors are:

- keep the props moving forward in clean air
- avoid sharp 180 degree flight reversals
- fly smoothly on arcs
- keep throttle up during turns 
- don't cut throttle to zero (except during inverted drops and the like)
...
- capable of rapidly changing thrust on demand
-- higher cell count, eg 6S is better than 4S
-- lighter, easily spun props
- heavier quads (more stable)
- battery on top (less wobble momentum)

From the software perspective:

- the less filter delay the better (though less filter delay = less noise filtering = hotter motors)
- usually, more D is better, to a point (lots of D can cause hotter motors)
- there is a 'sweet spot' for P, nether too low nor too high will be ideal
- improving PID responsiveness at very low rpm can be useful on some quads (thrust linear)

4.0 has been focused on stronger low rpm filtering to avoid jello on HD setups and to keep motors cool.  It has less high rpm filter delay than 3.5, but more low rpm filter delay.  Also D is running at a much lower value - the D_min value - most of the time.  That's the value that is active when you are flying smoothly, and it will only rise to the normal D value during active stick inputs.  D will also rise some of the way towards the full D value during propwash events, but not completely. On the other hand, the default 'max' D value is higher with Dain, 4.0 transmits D more effectively below the filter cutoffs than before, and the cutoff settings optimise D transfer at propwash frequencies much better than before.

Overall, in 4.0, propwash is typically less if throttle is sustained, especially above one-third to one-half throttle, but may be worse when pushing from low throttle.  Pilots who drop throttle to zero and fly with reverse 180's are likely to get more propwash than 3.5, but pilots who keep throttle up during turns are likely to find it better.And the results will vary a lot from quad to quad.  
...
- set D min to 20% below the D value from your old tune (so there is less difference between min and max D)

Once D's as good as you can get it, 
...
If the motors are cool, you could try to increase all those values say to 170, and fly again.  If you want to push them up higher, do so slowly, in small steps, cautiously.  Note that on a typical 5", 250hz equals mid-throttle rpm.  Be aware that the higher you go, the more likely the quad is to burn your motors if you fly with a bent prop or as your motors get bearing wobble.  Carefully check motor temps on moving these filter values.  You may also find that jello suddenly gets worse, or that motor temps abruptly get worse, as you increase these values.  

You may also consider rpm based filtering, which will give cleaner motor traces with less delay.  Note that this is still experimental at this point.

Be aware that the root cause of propwash is flight style.  Of the technical causes, the biggest factor, by far, is the delay arising from motor/prop thrust generation delay.  Only 10% of the problem is filter delay related.  Even if there was zero filter delay, there would still be propwash.  Most people will notice some improvement in propwash by lifting Dain and lifting the filters.  But at some point the 'returns' get less and less, and the risks of burning motors, and lost power, efficiency, and flight time, go against you.  Everybody's situation will be slightly different, and the compromises we make depend greatly on what we want to use the quad for.
...
Making sensible changes to these complex filters isn't easy.  A blackbox log in `set debug_mode = FFT_FREQ` mode is the only practical way.  We need to check that the FFT is tracking well, analyse the amount of noise and its frequency content at different throttle points, think about where it might be coming from, and how best to deal with it.  PID-Analyzer and the Blackbox spectrum analyser are two very useful tools.
...
### The following notes apply ONLY to un-integrated yaw configuration!  If you're flying [integrated yaw](https://github.com/betaflight/betaflight/wiki/Integrated-Yaw), the tuning process is completely different!
