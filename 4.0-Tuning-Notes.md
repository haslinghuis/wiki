...
- don't cut throttle to zero (except during flips, inverted drops and the like)
...
4.0 has been focused on stronger low rpm filtering to avoid jello on HD setups and to keep motors cool.  It has less high rpm filter delay than 3.5, but more low rpm filter delay.  **In 4.0 D is running at a much lower value - the D_min value - most of the time**.  That's the value that is active when you are flying smoothly.  D_min will only rise to the normal D value during active stick inputs.  D will also rise some of the way towards the full D value during propwash events, but not completely.  On the plus side, the default 'max' D value is higher with D_min, 4.0 transmits D more effectively below the filter cutoffs than before, and the cutoff settings optimise D transfer at propwash frequencies much better than before.
...
- set D_mins to 20% below the D values from your old tune (so there is less difference between min and max D)
