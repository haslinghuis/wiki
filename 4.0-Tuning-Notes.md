...
Filters in BF4.0 have become quite complex, so for some less experienced users it's hard to really distinguish between all available options. Here is a little writeup on the available filters and how/where to cofigure them, including the new dynamic rpm notch filters:
...
Note: Lowpass1 for gyro or dterm can be either dynamic or static, Lowpass2 is always static (reflected int he BFC GUI)
