...
Filters in BF4.0 have become quite complex, so for some less experienced users it's hard to really distinguish between all available options. Here is a little summary writeup on the available filters and how/where to configure them, including the new dynamic rpm notch filters:
