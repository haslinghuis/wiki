...
**Please read this important introduction**:
 ...
 1. When updating to Betaflight 4.3 you MUST use the **latest configurator
    found here:** 
    [latest nightly 10.8 Betaflight Configurator](https://github.com/betaflight/betaflight-configurator-nightlies)
    Earlier 10.8 and 10.7 configurator versions **will not** work
    properly.


 1. There is a **new slider tuning approach and presets** in order to make
    the firmware more accessible and easy to use. Due to parameter
    changes we **do not recommend you paste in CLI** ‘diff’ or ‘dumps’ from
    previous versions. We recommend to invest some time setting it up
    with this new approach and try the presets which should get great
    results much easier.

  

 1. **Experienced users** who still wish to use CLI for tuning and filters
    have to first disable the sliders in order to save those CLI inputs.
    **You must first input the command:** set simplified_tuning_mode = OFF
    or in the GUI toggle the switch above the slider from ‘RPY’ to
    ‘OFF’.

  

 1. **Check all your settings carefully** before attempting to arm for the
    first time.  Before arming: Check your Rates, motor configuration and rotation in the Configurator motors tab.  For the first flights, arm in a safe place, and fly gently to shake out any setup or build issues.

 3. **We recommend using RPM based filtering** for best performance.  Read about [enabling and configuring rpm filtering
...
**Overall a smoother, more precise flight experience with better propwash perfomance** 'and' also **easier to set up** - to release the full potential of Betaflight 4.
...
- [Presets](4.3-Tuning-Notes#Presets) - A fantastic, comprehensive new preset system!  Whether a whoop, a twig, a 5" racer, a freestyle setup, or an X-class, we will be providing a simple way apply a great tune for your quad, out of the box, as well as a host of useful setups for radio, vtx and other categories.  
- Changed overall default PID and filter values - less gyro filtering, higher P and D, to provide better out of the box performance.
...
- [Multi dynamic notch](4.3-Tuning-Notes#Multi-dynamic-notch) - Completely rewritten, much improved, SDFT based multi-dynamic notches.  More than one resonant peak can be tracked at the same time, more accurately and more quickly than before for very low latency cost. This allows for lower overall low pass filtering and better performance.
...
- [PT2 and PT3 lowpass filtering options](4.3-Tuning-Notes#PT2-and-PT3-lowpass-filtering-options) - The old biquad lowpass filter option is no longer available on Gyro, due to overshoot and resonance issues.  Previous biquad lowpass users should change to PT2 if they need second order filtering.

- [Feedforward jitter reduction](4.3-Tuning-Notes#Feedforward-jitter-reduction) - 4.3 introduces **feedforward jitter reduction**, which is an improvement on feedforward transition. It delivers a ‘dynamic transition’ effect to the feedforward, where you can get silky smooth responses to slow stick inputs, and immediate, snappy feedforward responses to quick inputs.  Jitter reduction also attenuates RC link noise during slow movements, especially for the newer higher rate Rx links. 
...
**4.3 should fly really well on defaults. Start there, experiment with the easy slider tuning, and check the presets to see if there is an applicable one there!**
