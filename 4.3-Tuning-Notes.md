## [Betaflight 4.3](https://github.com/betaflight/betaflight/releases) Introduction:

Welcome to a comprehensive guide to the Betaflight 4.3 update.

**Please read this important introduction**:

 1. When updating to Betaflight 4.3 you MUST use the [**latest nightly 10.8 Betaflight Configurator.**](https://github.com/betaflight/betaflight-configurator-nightlies) Earlier 10.8 and 10.7 configurator versions **will not** work properly.


 1. 4.3 has a **new Slider tuning approach and Presets.**  Due to parameter changes, **do not paste in CLI ‘diff’ or ‘dumps’ from previous versions**.  A suitable Preset should give great results, and the Sliders simplify making final 'tweaks' to the tune.

  
 1. **Experienced users** who want to use numeric data entry for tuning and filters MUST first disable the sliders in order to save those CLI inputs.  It's easiest to do that in Configurator.

  
 1. **Check all your settings carefully** before arming for the first time.  Check your PIDs, Filters, Rates, motor configuration and motor rotation in the Configurator motors tab.  For the first flights, arm in a safe place, and fly gently to shake out any setup or build issues.
...
## [Betaflight 4.3](https://github.com/betaflight/betaflight/releases) brings the following **updates**:
...
- We've changed how D is configured.  We now have Derivative (the old 'D_Min' value) and D_Max settings. The Derivative value is what sets the D value in smooth flying.  Using the same algorithm as before, when you turn fast, D rises to Dmax, providing more effective damping for fast movements while allowing less D noise and motor heat when flying straight.  The difference between the two values is less than before, and the Derivative value is significantly higher.  Unlike 4.2, you usually won't need to push the D slider higher, unless you are seeking stronger straight-line stability in a clean HD or cinematic build.

 - [More accurate loop times](4.3-Tuning-Notes#More-accurate-loop-times) - We've made massive scheduler and DMA code improvements, including EXTI triggered SPI gyro reads (on supported boards), giving super stable looptimes, smoother filter performance, faster logging and improved CPU efficiency.
...
Please refer to the [4.2 tuning guide](https://github.com/betaflight/betaflight/wiki/4.2-Tuning-Notes#settings-to-minimise-bounceback).  Most bounce-back is due to too much I on a low authority quad, and is usually best fixed by lowering the iterm_relax cutoff value.  Occasional wobbles at the end of some, but not all flip or rolls, especially the faster ones towards the end of the flight, are usually caused by partial desyncs, and these are best dealt with by enabling dynamic idle with a reasonably high minimum RPM value (eg 4000 RPM).

### Random wobbles in HD footage

This is typically an issue for cinematic HD applications.  The user may notice a kind of low level, random instability of the horizon when flying straight forward.  Mostly it means that the frame is too soft or flexy or it is being shaken by slightly out of balance props or worn bearings.  The following suggestions may help minimise this problem:
...
- use a jitter reduction value of 10-12.  To test if feed-forward is making it worse, do a test flight with feedforward at zero.  If there is no change, feed-forward isn't the problem, and can be used at normal settings.
...
- enable dynamic idle at 4000 RPM
...
2. The MPU6000 gyro has irregular updates with every 8th update taking not 125us, but 170us followed by another of 80us. As a result the gyro task would occasionally read two samples, 125us apart within the 170us interval and miss the update in the 80us cycle completely.
3. As the gyro's interval update and the gyro loop were asynchronous there was a random delay of up to 125us between the gyro data being updates and it being read/processed.

In 4.3 the above is fixed by:
1. Data is now transferred using Direct Memory Access (DMA) which means that the processor kicks off the transfer and can do other things whilst it waits for the transfer to complete.
2. The scheduler now syncs up to the gyro updates using the interrupt from the gyro (as defined by the `GYRO1_EXTI` resource) and so always captures every update.
3. Reads from gyro are now done immediately after an update and are processed a consistent amount of time afterwards.

Boards with a hardware external interrupt line from gyro to FC can benefit from the above changes.

The `status` CLI command will now report if DMA is being used (benefit 1 above) by including `dma` after the gyro line, and if the scheduler has locked to the gyro (benefits 2 and 3 above) by including `locked` after the gyro line thus:

```
Gyros detected: gyro 1 locked dma
```

The scheduler and tasks in 4.3 have also been revised to get much more accurate timing. Betaflight uses a cooperative multi-tasking scheme which means that the scheduler needs to know how long a task will take to run, so it will call it only when there is time before a vital operation such as running the gyro/filter/pid tasks. If a task takes longer to execute than predicted then other vital tasks will be late, and this was frequently the case in 4.2 especially for the RX and OSD tasks. 4.3 has greatly improved the behaviour of all tasks in completing within the expected time and the scheduler thus makes better scheduling decisions.
...
Blackbox logging over SPI is now much more efficient, so that higher logging rates will work with fewer gaps. This is achieved using DMA rather than polled access as described above. In 4.2 the processor would have to wait up to 90us for a block of data to be written to FLASH memory; in 4.3 is takes under 5us to kick off the write and then it completes in the background whilst the processor gets on with other tasks. Even so, logging is a significant time consumer, so turn it off if you don't need it, and only log as fast as is needed, eg 1kHz for normal purposes, 2Khz for filter tuning.
...
In order for the above gyro and FLASH improvements to work SPI DMA is needed. In the following example from an F745 FC it can be seen that `SPI_MOSI`/`SPI_MISO` are both defined for SPI bus 3 (used for FLASH access) and for SPI bus 1 (used for gyro access). Only `SPI_MOSI` is defined for bus 4, but that's OK as that's only used for the OSD which doesn't use `SPI_MISO`.
...
Unfortunately, due to an errata in the F4 processors ONLY (see https://www.st.com/resource/en/errata_sheet/dm00037591-stm32f405407xx-and-stm32f415417xx-device-limitations-stmicroelectronics.pdf, section 2.1.10) it is not possible to use DMA on SPI bus 1 and bit-banged DSHOT at the same time. Therefore if bit-banged DSHOT is enabled, you may see `locked` in the `status` output, but not `dma`. In these cases you may be better disabling bit-banged DSHOT, but this may result in DMA resource conflicts due to the `MOTOR 1` through `MOTOR 4` DMA descriptors conflicting with the SPI used for FLASH/OSD access.

For example, on a MATEKF11 with bit-banged DSHOT enabled we see SPI bus 1 and 2 enabled, but no gyro DMA.

```
# status
MCU F411 Clock=108MHz (PLLP-HSE), Vref=3.30V, Core temp=37degC
...
...
The new SDFT based notch can track multiple independent noise peaks, and will assign an independent notch filter to each peak.  The user can specify as many notches as they need.  Usually no more than two or three are needed, depending on the build, and often only one.

The SDFT tracks noise peaks much more effectively than before.  The code is completely different.  It has been optimised to control resonance problems when used in association with RPM filtering.  The RPM filter is still best for RPM related noise.
...
A suitable starting value of 25 is adequate for most builds.  Higher values - eg 50 - are typically needed for most 250hz links, and 60-65 for 500hz links.

`feedforward_averaging` is now set to 'off' by default, since most builds won't require it.  It should be set to `3_POINT` to provide three point moving averaging for old Crossfire 150 links.  Noisy 500hz RC links may benefit from `2_POINT` averaging, but the new jitter reduction code means that most 250hz and lower links are usually OK without averaging.
