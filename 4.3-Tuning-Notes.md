...
- [New PID based tuning sliders](4.3-Tuning-Notes#New-tuning-sliders) - Whether you need to tweak the tune or build it from scratch, we now have simpler, more comprehensive, firmware based tuning sliders in the Configurator.  Slider positions are stored with the quad, and can be modified via the OSD.  New expert sliders allow fine-tuning of Pitch:Roll balance, DMax:Dmin, and I relative to P.  
- Default PID and filter values have changed slightly to reflect better default flight stability.
...
**NOTE 1**:The [latest 10.8 Betaflight Configurator](https://github.com/betaflight/betaflight-configurator/releases) is MANDATORY.  Earlier configurator versions will not work properly!
...
**NOTE 4**: We strongly recommend flying the default antigravity value of 3.0 and only trying higher values if testing shows that there is a definite improvement [see](4.3-Tuning-Notes#AntiGravity-improvements).  
...
Future presets are also under development that can incorporate Rate Banks, Aux Channel and Modes, OSD Layouts, LED Configurations, and more.
...
To increase combined P&D gain while retaining, approximately, the P:D ratio, move both sliders to the right in equal amounts.  The P:D ratio will change a little bit despite changing P and D sliders in equal steps on both P and D sliders, and in any case the optimal P:D ratio will change with overall gain, so some fine 'balance' tuning may still be required after changing overall gain.

A simple way to tune clean quads is just to move both P and D sliders together to the right until something 'bad' happens.  Either motors get hot or the quad makes bad noises in high-speed turns, then back off a bit.  For quick and simple tuning this is often all you need to do.

Another way to tune - the "D first" approach - is based on the fact that that the more D you can apply, the more P you can use, and that generally, the quad will track best with the highest P and D it can tolerate.  The limiting factor in most builds is the amount of D you can apply before the motors get hot, the quad starts to grind or resonate or it makes bad noises from D oscillations.  The 'D first' tuning method involved stepwise increases in D until something bad happens, then easing back a click or two, and not going higher.  You may find that different props or ESC settings permit higher D, but we know that the primary limit on how much D you can run is how clean the build is - builds with good bearings, well balanced and well centred motors, stiff frames, stiff props, etc, will accept more D.   Once you have D as high as it can reasonably go, bring P up until something bad happens, typically a fluttering sound from P oscillation in hard high throttle turns or under-damped short duration overshoots or resonances in step responses.  Usually most quads end up with P somewhere close to or up to 50% higher than D.  

The 'classical' tuning method is to leave D where it is, or drop it back a bit, and then stepwise push P higher one click at a time until you hear that P mediated flutter or oscillation in high-speed turns.  Once you know what 'too much P' does, you can put it just below that point, and ease D up to see if you can attenuate that problem without adding too much motor heat from D related noise or resonance.  Once you know the D limit, you can again see if you can bring P up a little higher, and often you can since you'll have more damping.  The goal is to get both parameters as high as you can without the unwanted issues of flutter, resonance or excessive motor heat.
