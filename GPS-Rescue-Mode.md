...
## Common pitfalls
- Ensure that you are flying further than the minimum distance to home (100m by default) before testing GPS Rescue. The minimum distance can be set in the CLI using `set gps_rescue_min_dth = <meters>`. If GPS rescue is activated at less than the minimum distance the quad will drop.
