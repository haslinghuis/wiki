...
v4.1 and beyond - After the RC link is restored, Betaflight now requires that any combination of roll/pitch/yaw inputs be moved beyond the failsafe_stick_threshold percentage value.   The default for this setting is 30 which means you need to move roll+pitch+yaw at least 30% off center before it will cancel Failsafe and return control.   You may choose to reduce that setting value if you need a less abrupt transition from failsafe to full control (iNav defaults to 10%). 
