...
To use BLHeli Pass-through first set to OneShot in the config tab and Save. Then in the CLI set motor 1 back to the the original processor pin with the resource command. Save.  The wire from motor1 pin to the PPM pin does not need to be removed since the PPM pin is now 'undefined' and is set to an input. BLHeli Suite can now be used to update ESC firmware and/or make changes to settings. When finished set Motor1 to the DMA channel on pin A07, Save. then select Dshot.
